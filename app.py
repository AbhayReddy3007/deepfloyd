...
# ---------------- HUGGING FACE (DeepFloyd IF for Images) ----------------
import torch
...
with tab_gen:
    ...
    if st.button("🚀 Generate"):
        if not raw_prompt.strip():
            ...
            with st.spinner("Generating with DeepFloyd IF..."):
                generated = []
...
with tab_edit:
    ...
    if st.button("🚀 Edit"):
        if not uploaded_file or not edit_prompt.strip():
            ...
            with st.spinner("Editing with DeepFloyd IF..."):
                edited_versions = []
                ...
                for idx, out_bytes in enumerate(edited_versions):
                    ...
                    st.image(Image.open(BytesIO(out_bytes)), caption=fname, use_column_width=True)
